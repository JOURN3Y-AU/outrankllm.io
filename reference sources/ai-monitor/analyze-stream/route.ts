...
export async function POST(request: Request) {
  ...
  const runAnalysis = async () => {
    try {
      ...
      if (!session) {
        await sendProgress({ type: 'error', message: 'Unauthorized' })
        await writer.close()
        return
      }

      await sendProgress({ type: 'progress', step: 'starting', message: 'Initializing analysis...' })
      ...
      if (runError || !latestRun) {
        await sendProgress({ type: 'error', message: 'No completed runs found' })
        await writer.close()
        return
      }
      ...
      if (responsesError || !responses) {
        await sendProgress({ type: 'error', message: 'Failed to fetch responses' })
        await writer.close()
        return
      }
      ...
      // Run AI analyses in parallel with progress updates
      await sendProgress({ type: 'progress', step: 'ai_analysis', message: 'Querying all 3 AI platforms in parallel...' })
      ...
      if (chatgptResult.usage) costRecords.push(createCostRecord('chatgpt_analysis', chatgptResult.usage))
      if (claudeResult.usage) costRecords.push(createCostRecord('claude_analysis', claudeResult.usage))
      if (geminiResult.usage) costRecords.push(createCostRecord('gemini_analysis', geminiResult.usage))

      // Synthesize
      await sendProgress({ type: 'progress', step: 'synthesis', message: 'Synthesizing recommendations...' })
      const synthesisResult = await synthesizeWithClaude(
        chatgptResult.content,
        claudeResult.content,
        geminiResult.content
      )
      if (synthesisResult.usage) costRecords.push(createCostRecord('synthesis', synthesisResult.usage))
      ...
      // Save analysis
      await sendProgress({ type: 'progress', step: 'saving', message: 'Saving analysis...' })
      ...
      if (savedAnalysis?.id && costRecords.length > 0) {
        ...
        await supabase.from('ai_monitor_api_costs').insert(costInserts)
      }

      // Generate PRD
      if (savedAnalysis?.id) {
        await sendProgress({ type: 'progress', step: 'prd_generation', message: 'Generating PRD tasks...' })

        try {
          // Pass site pages context to PRD generator for current state awareness
          ...
          if (prdResult.usage) {
            ...
            await supabase.from('ai_monitor_api_costs').insert({
              analysis_id: savedAnalysis.id,
      ...
      // Send final result
      await sendProgress({ type: 'progress', step: 'complete', message: 'Analysis complete!' })
      await sendProgress({
        type: 'result',
        data: {
      ...
      await sendProgress({ type: 'error', message: 'Internal server error' })
      await writer.close()
    }
  }
