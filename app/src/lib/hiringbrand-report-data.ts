...
/**
 * Rescale the raw differentiation score for display.
 *
 * Raw scores are generated by converting Claude's 1-10 sub-scores linearly to 0-100,
 * but Claude rarely awards extreme values — the real-world ceiling is ~55. This compresses
 * everyone into the bottom quarter and makes meaningful differences invisible.
 *
 * The sqrt transform stretches the low end so that a genuinely good score (raw ~30,
 * ~85th percentile across all scanned brands) reads as solid (74%) rather than poor.
 * Bad scores (raw 6-10) still display in the 30-40% range — clearly below par.
 *
 * Applied only at the display layer; the stored DB value is unchanged so trends compare correctly.
 */
function scaleDifferentiationScore(raw: number | null | undefined): number | null {
  if (raw == null) return null
  return Math.min(100, Math.round(Math.sqrt(raw / 55) * 100))
}
...
async function getTrendsData(
  ...
  const scoreHistory: HBScoreHistoryEntry[] = Array.from(scoreByDay.values())
    ...
    .map((h) => ({
      ...
      differentiationScore: scaleDifferentiationScore(h.differentiation_score),
...
export async function fetchHBReportData(token: string): Promise<HBReportDataWithExtras | null> {
  ...
  return {
    report: {
      ...
      differentiationScore: scaleDifferentiationScore((report as { differentiation_score?: number }).differentiation_score),
